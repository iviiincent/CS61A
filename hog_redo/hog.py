...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    return (
        piggy_points(opponent_score) if num_rolls == 0 else roll_dice(num_rolls, dice)
    )
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for roll_number in range(1, 11):
        cur_score = make_averaged(roll_dice, trials_count)(roll_number, dice)
        if cur_score > highest_score:
            highest_roll = roll_number
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    cur_score = piggy_points(opponent_score)
    if cur_score >= cutoff or more_boar(cur_score + score, opponent_score):
        return 0
    return num_rolls
